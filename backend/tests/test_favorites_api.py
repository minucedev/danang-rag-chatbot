...
async def test_add_then_list_camelcase_envelope(tmp_db):
    await add_favorite(
        FavoriteCreate(client_id="c1", point_id="p1", collection="places_danang", snapshot=_snap())
    )
    res = await list_favorites(client_id="c1")
...
async def test_list_empty(tmp_db):
    assert await list_favorites(client_id="nobody") == {"items": [], "total": 0}
...
async def test_list_failure_raises_500(monkeypatch, tmp_db):
    ...
    with pytest.raises(HTTPException) as ei:
        await list_favorites(client_id="c1")
